...
class Application:
    ...
    __contiguous_cloud_copy_success_count = 0

    def __init__(self):
        self.__version = Version(1, 3, 0)
        self.__mirror_version = Version(1, 3, 0)
        self.__reaper_version = Version(1, 2, 0)
    ...
    def __init_cli_support(self):
        ...
        self.__parser.add_argument("--reaper-interval", action="store", required=False, type=float,
                                   help="the amount of time in seconds between checking files to reap", default=os.environ.get('REAPER_INTERVAL'))#default="60.0")
        self.__parser.add_argument("--reaper-default-backoff", action="store", required=False, type=float,
                                   help="the default amount of time in seconds to backoff between cloud uploads",
                                   default=os.environ.get('REAPER_DEFAULT_BACKOFF'))#default="0.5")
        self.__parser.add_argument("--reaper-backoff-increment", action="store", required=False, type=float,
                                   help="the amount of time in seconds to increment or decrement from the backoff interval",
                                   default=os.environ.get('REAPER_BACKOFF_INCREMENT'))#default="10.0")
        self.__parser.add_argument("--reaper-success-threshold", action="store", required=False, type=int,
                                   help="the number of contiguous successes before reducing the backoff",
                                   default=os.environ.get('REAPER_SUCCESS_THRESHOLD'))#default="10")
        ...
        if self.__arguments.reaper_default_backoff is None or self.__arguments.reaper_default_backoff <= 0:
            self.__arguments.reaper_default_backoff = 0.5

        if self.__arguments.reaper_backoff_increment is None or self.__arguments.reaper_backoff_increment <= 0:
            self.__arguments.reaper_backoff_increment = 10.0

        if self.__arguments.reaper_success_threshold is None or self.__arguments.reaper_success_threshold <= 0:
            self.__arguments.reaper_success_threshold = 10
    ...
    def reaper_default_backoff(self):
        return self.__arguments.reaper_default_backoff

    def reaper_backoff_increment(self):
        return self.__arguments.reaper_backoff_increment

    def reaper_success_threshold(self):
        return self.__arguments.reaper_success_threshold
    ...
    def cloud_copy(self, logger, storage, remote_path, filename, reaper=None):
        start_time = None
        ...
        try:
            ...
            # increment contiguous success counter, check continguous success counter to see if we can reduce reaper backoff
            if upload_result is True:
                self.__contiguous_cloud_copy_success_count += 1
                if reaper is not None and reaper.backoff() > self.reaper_default_backoff() and \
                    self.__contiguous_cloud_copy_success_count > self.reaper_success_threshold():
                    reaper.decrement_backoff()
                    logger.debug("Cloud Copy, contiguous cloud copy success count is greater than threshold, decrementing reaper backoff interval")
            else:
                self.__contiguous_cloud_copy_success_count = 0
                reaper.increment_backoff()
                logger.warning("Cloud Copy Failed, incrementing reaper backoff interval")

            return upload_result

        except Exception as exception:
            self.__contiguous_cloud_copy_success_count = 0
            if SLOW_DOWN in str(exception):
                if reaper is not None:
                    reaper.increment_backoff()
                logger.warning(
                    (
                            "Cloud Copy " + SLOW_DOWN + " Response [ service = '%s', watch_directory = '%s', bucket = '%s', remote_path = '%s', filename = '%s' ]"),
                    storage.service_name(), self.monitored_path(), storage.bucket_name(), remote_path, filename)
            else:
                logger.exception(
                    "Cloud Copy Error [ service = '%s', watch_directory = '%s', bucket = '%s', remote_path = '%s', filename = '%s' ]: %s",
                    storage.service_name(), self.monitored_path(), storage.bucket_name(), remote_path, filename, exception)
            return False
        finally:
            if self.debug() and start_time is not None and verify_start_time is not None:
                end_time = time_ns()
    ...
    def cloud_hash(self, logger, storage, path, filename):
        ...
        try:
            ...
            logger.exception(
                "Cloud Hash Computation Error [ service = '%s', bucket = '%s' remote_path = '%s', filename = '%s' ]: %s",
                storage.service_name(), storage.bucket_name(), path, filename, exc_info=exception)
            return None
        finally:
    ...
    def opposing_filename(self, filename):
        if filename is not None and filename.endswith(self.signature_extension()):
            return self.filename_change_extension(filename, self.signature_extension(), self.record_extension())
        elif filename is not None and filename.endswith(self.record_extension()):
            return self.filename_change_extension(filename, self.record_extension(), self.signature_extension())

        return filename

    def filename_change_extension(self, filename, current_extension, new_extension):
        match = self.filename_remove_extension(filename, current_extension)
        if match is not None:
            return str.format("{}.{}", match, new_extension)

        return filename

    def filename_remove_extension(self, filename, current_extension):
        expr = str.format("(.+)\\.{}$", re.escape(current_extension))
        match = re.search(expr, filename)
        if match is not None:
    ...
    def run(self):
        try:
            ...
            self.__logger.debug("Monitored Path: %s", self.monitored_path())
            ...
            while self.__running:
                ...
                if self.__reaper is not None:
                    if not self.__reaper.is_alive():
                        self.__logger.warning("Reaper Death Detected - Restarting [ min_keep = '%s', interval = '%s', default_backoff = '%s', backoff_increment = '%s', reaper_success_threshold = '%s' ]",
                                              self.reaper_min_keep(),
                                              self.reaper_interval(),
                                              self.reaper_default_backoff(),
                                              self.reaper_backoff_increment(),
                                              self.__arguments.reaper_success_threshold)
                        self.__reaper = Reaper(self, self.__reaper_version)
                        self.__reaper.start()
...
class Reaper(Thread):
    ...
    def __init__(self, application, version):
        ...
        self.__backoff = self.__application.reaper_default_backoff()
    ...
    def increment_backoff(self):
        self.__backoff += self.__application.reaper_backoff_increment()

    def decrement_backoff(self):
        new_backoff = self.backoff() - self.__application.reaper_backoff_increment()
        if new_backoff >= self.__application.reaper_default_backoff():
            self.__backoff = new_backoff
        else:
            self.__backoff = self.__application.reaper_default_backoff()
        ...
        try:
            with os.scandir(self.__application.monitored_path()) as file_list:
                for entry in file_list:
                    ...
                    if self.__application.require_signature():
                        key = self.__application.filename_remove_extension(filename, self.__record_extension)
                        if key is None:
                            key = self.__application.filename_remove_extension(filename, self.__signature_extension)
                    if key is None:
                        self.__logger.warning("Collected Work Item key not created successfully, [ filename = '%s' ]", filename)
    ...
    def __invoke_cloud_copy(self, storage, remote_path, filename):
        thread = Thread(target=self.__application.cloud_copy, args=(self.__logger, storage, remote_path, filename, self))
        thread.daemon = True
        thread.start()
    ...
    def run(self):
        try:
            self.__logger.info(
                "%s Starting [ record_extension = '%s', signature_extension = '%s', require_signature = '%s', min_keep = '%s', interval = '%s',  default_backoff = '%s', backoff_increment = '%s', reaper_success_threshold = '%s']",
                self.__version.to_version_string("Reaper"), self.__record_extension, self.__signature_extension,
                self.__require_signature, self.__min_keep, self.__interval, self.__application.reaper_default_backoff(),
                self.__application.reaper_backoff_increment(),
                self.__application._Application__arguments.reaper_success_threshold)
            ...
            while self.__running:
                ...
                if work_items is None or len(work_items) <= 0:
                    ...
                    self.__logger.debug("Reaper Collected Work Items [ work_items length = %s ]", len(work_items))
...
class MirrorFileEventHandler(RegexMatchingEventHandler):
    ...
    def __init__(self, application, regexes=(".*"), ignore_regexes=(), ignore_directories=False,
        ...
        self.__logger = logging.getLogger("Mirror")
        self.__console_logger = logging.getLogger("Console")
        self.__logger.debug("MirrorFileEventHandler Initialized")
...
class RemoteStorage:
    ...
    def put(self, local_path, key):
        try:
            ...
            self.__logger.exception(
                "Remote Put Error FileNotFoundError: error: %s, filename: %s",
                fnf_error.args[1], fnf_error.filename, exc_info=fnf_error)
            return False
        except ClientError as ex:
            self.__logger.exception(
